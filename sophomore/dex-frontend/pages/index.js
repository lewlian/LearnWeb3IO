		...
		if (liquidityTab) {
            ...
            we calculate based on the `Eth` amount specified by the user how much `CD` tokens can be added */}
						{utils.parseEther(reservedCD.toString()).eq(zero) ? (
							<div>
								...
								<input type='number' placeholder='Amount of CryptoDev tokens' onChange={(e) => setAddCDTokens(BigNumber.from(utils.parseEther(e.target.value || '0')))} className={styles.input} />
			...
			return (
				<div>
					...
					<div className={styles.inputDiv}>
						{/* Convert the BigNumber to string using the formatEther function from ethers.js */}
						{ethSelected ? `You will get ${utils.formatEther(tokenToBeRecievedAfterSwap)} Crypto Dev Tokens` : `You will get ${utils.formatEther(tokenToBeRecievedAfterSwap)} Eth`}
					</div>
